...
type JobState =
  | { ok: true; status: "running"; progress: number; message: string; detail?: any; createdAt: number }
  | { ok: true; status: "done"; progress: number; message: string; result: any; createdAt: number }
  | { ok: false; status: "error"; progress: number; message: string; createdAt: number };
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const setState = (st: Omit<JobState, "createdAt">) => {
      m.set(jobId, { ...(st as any), createdAt } as JobState);
    };
